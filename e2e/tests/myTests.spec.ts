...
import { errorMessages } from "e2e/constants/checkoutInformationErrorMessages";
...
test.describe("Five test scenarios", () => {
  ...
  test("Remove a product from the cart page", async ({ page }) => {
    ...
    await expect(removeButton(page)).not.toBeVisible();
    await expect(cartIcon(page)).toBeVisible();
    await expect(productInCart(page)).not.toBeVisible();
  });
